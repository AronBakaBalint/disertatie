...
public class ReservationRepositoryImpl implements ReservationRepository {
	...
	@Override
	@SuppressWarnings("unchecked")
	public Optional<Reservation> findByDetectedLicensePlate(String licensePlate) {
		List<Reservation> reservations = entityManager
				.createQuery("SELECT r FROM Reservation r left join fetch r.parkingSpot p WHERE r.licensePlate = :licensePlate")
				.setParameter("licensePlate", licensePlate).getResultList();
		return reservations.isEmpty() ? Optional.empty() : Optional.of(reservations.get(0));
	}
